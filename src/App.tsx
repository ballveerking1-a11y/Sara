...
- MINI-GAMES: You can play exciting games with Mahendra! Use the 'openMiniGame' tool to start.
...
export default function App() {
  ...
  const handleSendTextMessage = () => {
    if (!textMessage.trim() || !liveSessionRef.current) return;
    setTranscription(prev => ({ ...prev, user: textMessage.trim() }));
    try {
      liveSessionRef.current.sendRealtimeInput({
  ...
  const startMahi = async () => {
    try {
      ...
      let micPermission: MediaStream | null = null;
      try {
        ...
        console.warn('Microphone permission denied, starting in text-only fallback mode:', micErr);
        ...
        callbacks: {
          onopen: () => {
            ...
            if (micPermission) {
              ...
              console.log('Connected to Mahi in text mode!');
            }
          },
  ...
  return (
    <div className="fixed inset-0 bg-[#000000] flex flex-col items-center justify-center overflow-hidden font-sans text-white">
      ...
      <div className="absolute bottom-0 left-0 right-0 min-h-[240px] bg-gradient-to-t from-black via-black/95 to-transparent flex flex-col items-center justify-end pb-12 pt-16 px-4 z-40">
        ...
        {isActive && (
          <div className="mb-5 flex items-center gap-2 w-full max-w-sm px-2 pointer-events-auto">
            <input
              ...
              className="flex-1 bg-black/60 border border-white/10 rounded-xl px-3.5 py-2 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-pink-500/50 transition-all font-sans shadow-inner"
              id="m-chat-input"
            />
            <motion.button
              ...
              className="px-3.5 py-2 bg-gradient-to-r from-pink-500 to-indigo-500 text-white font-bold rounded-xl text-[10px] uppercase tracking-wider hover:opacity-90 transition-opacity cursor-pointer shrink-0 animate-pulse"
              id="m-send-btn"
            >
              Send
            </motion.button>
        ...
        <div className="flex items-center gap-1.5 h-[50px] mb-6">
          <AnimatePresence>
            {isSpeaking ? (
              [...Array(12)].map((_, i) => (
                <motion.div
        ...
        <AnimatePresence>
          {showGameMenu && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 15 }}
              className="absolute bottom-32 bg-[#0e0e11]/95 border border-white/10 backdrop-blur-xl p-4 rounded-2xl w-72 flex flex-col gap-2.5 shadow-[0_10px_40px_rgba(0,0,0,0.8)] z-50 pointer-events-auto"
              id="mini-game-launcher"
            >
              <div className="text-[10px] font-bold uppercase tracking-widest text-gray-400 border-b border-white/5 pb-1.5 flex justify-between items-center">
                <span>Select a Mini-Game</span>
                <span className="text-pink-400 flex items-center gap-1"><Sparkles size={10} /> Play with Mahi</span>
              </div>
              <div className="flex flex-col gap-1.5 text-white">
                <button
                  onClick={() => { setGameMode('ludo'); setShowGameMenu(false); }}
                ...
                >
                  <div className="w-7 h-7 rounded-lg bg-pink-500/15 flex items-center justify-center text-pink-400 group-hover:scale-110 transition-transform shrink-0">
                    <Trophy size={14} />
                  </div>
                  <div>
                    <div className="font-bold">Mahi's Neon Ludo</div>
                    <div className="text-[9px] text-gray-400">A sweet racing game</div>
                  </div>
                </button>

                <button
                  onClick={() => { setGameMode('trivia'); setShowGameMenu(false); }}
                  className="flex items-center gap-3 p-2 hover:bg-white/5 rounded-xl transition-all text-left text-xs cursor-pointer group w-full"
                >
                  ...
                  <div>
                    <div className="font-bold">Sweet Trivia Quiz</div>
                    <div className="text-[9px] text-gray-400">For happy & praised moods</div>
                  </div>
                </button>

                <button
                  onClick={() => { setGameMode('sound'); setShowGameMenu(false); }}
                  className="flex items-center gap-3 p-2 hover:bg-white/5 rounded-xl transition-all text-left text-xs cursor-pointer group w-full"
                >
                  <div className="w-7 h-7 rounded-lg bg-indigo-500/15 flex items-center justify-center text-indigo-400 group-hover:scale-110 transition-transform shrink-0">
                    <Music size={14} />
                  </div>
                  <div>
                    <div className="font-bold">Acoustic Mystery</div>
                    <div className="text-[9px] text-gray-400">Guess the Sound! Cheers you up!</div>
                  </div>
                </button>
                ...
                <button
                  ...
                  className="w-full py-2 bg-gradient-to-r from-pink-500/20 to-indigo-500/20 hover:from-pink-500/30 hover:to-indigo-500/30 border border-pink-500/20 rounded-xl font-bold text-[10px] tracking-wider uppercase text-pink-300 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
                  <Sparkles size={12} className="text-yellow-400 animate-bounce" />
                  <span>Ask Mahi to Pick</span>
                </button>
              </div>
            </motion.div>
          )}
        ...
        <div className="flex items-center gap-6 relative z-50">
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.1, backgroundColor: showGameMenu ? `${theme.primary}4D` : `${theme.primary}33` }}
            whileTap={{ scale: 0.95 }}
            className={`
              w-14 h-14 rounded-2xl border flex items-center justify-center cursor-pointer
              bg-black/60 shadow-[0_4px_20px_rgba(0,0,0,0.5)] backdrop-blur-lg
              transition-all duration-300 group
              ${showGameMenu ? 'shadow-[0_0_20px_rgba(0,0,0,0.2)]' : 'hover:border-white/40'}
            `}
            style={{ 
              borderColor: showGameMenu ? theme.secondary : `${theme.primary}4D`,
              boxShadow: showGameMenu ? `0 0 20px ${theme.glow}` : 'none'
            }}
            title="Mini Games"
          >
